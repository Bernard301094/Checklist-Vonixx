 ...
 * Agrupamento inteligente por Data -> Colaborador -> Máquina/Secção
 */
import React, { useState, useMemo } from 'react';
import {
  AlertTriangle, CheckCircle2, TrendingUp, Users,
  Clock, Calendar, Camera, ChevronRight, X, ChevronDown, ChevronUp,
  BarChart2, Activity, Shield, LayoutDashboard, Search, Cpu, User, Layers
} from 'lucide-react';
...
export default function DashboardView({
  ...
  const occurrencesByDate = useMemo(() => {
    ...
    return Object.entries(dateMap)
      ...
      .map(([date, reporters]) => ({
        date,
        reporters: Object.entries(reporters).sort((a,b)=>b[1].length - a[1].length),
      }));
  }, [occurrences, searchTerm]);

  /* ================== DADOS DE CONFORMIDADES ================== */
  const conformByDate = useMemo(() => {
    const dateMap: Record<string, Record<string, { total: number; checked: number }>> = {};
    
    // Simplificando: Assumimos os dados do checklist diário focados no dia atual para evitar excesso de loops no histórico completo
    const dKey = todayKey(); 
    dateMap[dKey] = {};
    ...
    checklistEntries.forEach(entry => {
      ...
      if (rep) {
        if (!dateMap[dKey][rep]) dateMap[dKey][rep] = { total: totalItems, checked: 0 };
        if (entry.is_checked) dateMap[dKey][rep].checked += 1;
      }
    });
    ...
    return Object.entries(dateMap)
      ...
      .map(([date, reporters]) => ({
        date,
        reporters: Object.entries(reporters).map(([name, stats]) => ({
          name,
          ...stats,
          pct: Math.round((stats.checked / stats.total) * 100) || 0
        })).sort((a,b)=>b.pct - a.pct),
      }));
  }, [checklistEntries, totalItems]);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px', padding: '16px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '16px' }}>
            ...
            <div className="card" style={{ padding: '20px', background: 'linear-gradient(180deg, var(--surface) 0%, var(--surface-2) 100%)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <div style={{ width: 44, height: 44, borderRadius: '12px', background: 'var(--warning-hl)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--warning)' }}>
            ...
            <div className="card" style={{ padding: '20px', background: 'linear-gradient(180deg, var(--surface) 0%, var(--surface-2) 100%)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <div style={{ width: 44, height: 44, borderRadius: '12px', background: 'var(--success-hl)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--success)' }}>
                  <TrendingUp size={20} />
                </div>
              </div>
              <div style={{ marginTop: '16px' }}>
            ...
            <div className="card" style={{ padding: '20px', background: 'linear-gradient(180deg, var(--surface) 0%, var(--surface-2) 100%)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <div style={{ width: 44, height: 44, borderRadius: '12px', background: 'rgba(8,145,178,0.12)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#0891b2' }}>
                  <Users size={20} />
                </div>
              </div>
              <div style={{ marginTop: '16px' }}>
      ...
      {tab === 'alertas' && (
        <div style={{ animation: 'fadeIn 0.3s ease-out' }}>
          <div style={{ position: 'relative', marginBottom: '24px' }}>
            ...
            <input
              type="text"
              placeholder="Pesquisar por área, problema ou operador..."
              value={searchTerm}
                ...
                <div key={date}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '16px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '6px 16px', borderRadius: '999px', background: date === todayKey() ? 'var(--primary-hl)' : 'var(--surface)', border: `1px solid ${date === todayKey() ? 'rgba(1,105,111,0.2)' : 'var(--border)'}`, color: date === todayKey() ? 'var(--primary)' : 'var(--text)', fontSize: '12px', fontWeight: 900, textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                      <Calendar size={14} /> {dateLabel(date)}
                    </div>
      ...
      {/* ─ Conformidades (Agrupado) ─────────────────────── */}
      {tab === 'conformidades' && (
        <div style={{ animation: 'fadeIn 0.3s ease-out' }}>
          {conformByDate.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '60px 24px', background: 'var(--surface-2)', borderRadius: '16px', border: '2px dashed var(--border)' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
              {conformByDate.map(({ date, reporters }) => (
                <div key={date}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '16px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '6px 16px', borderRadius: '999px', background: 'var(--surface)', border: '1px solid var(--border)', color: 'var(--text-muted)', fontSize: '12px', fontWeight: 900, textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                      <Calendar size={14} /> {dateLabel(date)}
                    </div>
                    <div style={{ flex: 1, height: 1, background: 'var(--border)' }} />
                  </div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '16px' }}>
                    {reporters.map(rep => (
                      <div key={rep.name} className="card" style={{ padding: '20px', display: 'flex', alignItems: 'center', gap: '16px', background: 'var(--surface)', border: '1px solid var(--border)' }}>
                        <div style={{ width: 48, height: 48, borderRadius: '50%', background: avatarColor(rep.name), color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, fontWeight: 900, boxShadow: `0 4px 12px ${avatarColor(rep.name)}44` }}>
                          {initials(rep.name)}
                        </div>
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 6 }}>
                            <div style={{ flex: 1, height: 6, background: 'var(--divider)', borderRadius: 99, overflow: 'hidden' }}>
                          ...
                          <div style={{ fontSize: 11, color: 'var(--text-muted)', fontWeight: 600, marginTop: 4 }}>
                            {rep.checked} de {rep.total} verificados
                          </div>
                        </div>
                      </div>
                    ))}
    ...
    <div className="card animate-in" style={{ padding: '16px', background: open ? 'var(--surface-2)' : 'var(--surface)', border: open ? '1px solid var(--primary)' : '1px solid var(--border)', transition: 'all 0.2s' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer' }} onClick={() => setOpen(!open)}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
          <div style={{ width: 44, height: 44, borderRadius: '12px', background: color, color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, fontWeight: 900, boxShadow: `0 4px 12px ${color}40` }}>
            {initials(reporter)}
          </div>
          <div>
            <div style={{ fontSize: '15px', fontWeight: 800, color: 'var(--text)', fontFamily: 'var(--font-display)' }}>{reporter}</div>
            <div style={{ display: 'flex', gap: '8px', marginTop: 4 }}>
              <span style={{ fontSize: '11px', fontWeight: 800, background: 'var(--warning-hl)', color: 'var(--warning)', padding: '2px 8px', borderRadius: '6px' }}>{occs.length} Alertas</span>
            </div>
          </div>
        </div>
        <div style={{ color: 'var(--text-muted)', padding: '6px', borderRadius: '8px', background: 'var(--surface-2)' }}>
            ...
            <div key={occ.id} style={{ padding: '14px', borderRadius: '12px', background: 'var(--surface)', border: '1px solid var(--border)', boxShadow: 'var(--sh-sm)' }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '12px' }}>
                <div style={{ display: 'flex', gap: '12px' }}>
                  <div style={{ width: 32, height: 32, borderRadius: '8px', background: 'var(--warning-hl)', color: 'var(--warning)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                  ...
                  <div>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 6 }}>
                      <span style={{ fontSize: 10, fontWeight: 800, background: 'rgba(37,99,235,0.1)', color: '#2563eb', padding: '2px 6px', borderRadius: 4, textTransform: 'uppercase' }}>
                        <Cpu size={10} style={{ display: 'inline', marginRight: 4 }} /> {machineLabel(occ.reporter) || 'MÁQUINA N/A'}
                      </span>
                    </div>
                  </div>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: '11px', fontWeight: 700, color: 'var(--text-muted)' }}>
                  <Clock size={12} /> {occ.time}
                </div>
              </div>

              {occ.comment && (
                <div style={{ padding: '12px', background: 'var(--surface-2)', borderRadius: '8px', border: '1px dashed var(--border)', marginTop: '12px' }}>
                  <p style={{ fontSize: '13px', color: 'var(--text)', fontWeight: 500, margin: 0 }}>"{occ.comment}"</p>
                </div>
              )}
              ...
              {occ.photos.length > 0 && (
                <div style={{ display: 'flex', gap: '8px', marginTop: '12px', overflowX: 'auto', paddingBottom: 4 }}>
                  {occ.photos.map((p, i) => (
                    <button key={i} type="button" onClick={() => onOpenPhoto(occ.photos, i)} style={{ width: 60, height: 60, borderRadius: '8px', overflow: 'hidden', border: '1px solid var(--border)', flexShrink: 0, padding: 0, cursor: 'zoom-in' }}>
